...
class PowerUpFactory {
	...
	public static ArrayList<PowerUp> getAllPowerUps() {
		...
		all.add(new PowerUp("Hill", "吸血能力", "攻撃時HP回復", "威力低下",
				"【解説】弾が命中するたびに少量のHPを回復する。\n【運用】威力は下がるため、一撃の重さより手数を優先せよ。\n体力を削り合う消耗戦に持ち込めば確実に競り勝てる。") {
			public void apply(Player p) { p.weapon.addEffect(new EffectHill()); }});

		all.add(new PowerUp("Rising", "高速弾", "弾速+100%", "威力-20%",
				"【解説】弾速が倍になり、敵が反応して避けるのが困難になる。\n【運用】遠距離からの狙撃に最適。\n威力が下がる分、確実に当て続けるエイム力が問われる。") {
			public void apply(Player p) { p.weapon.addEffect(new EffectRising()); }});

		all.add(new PowerUp("Impact", "高威力砲", "威力・弾速2倍", "間隔・リロード増",
				"【解説】単発火力が劇的に向上するが、連射が効かなくなる。\n【運用】無駄撃ちは厳禁。相手の硬直を狙って撃つこと。\n「Shotgun」などと組み合わせるとリスクを軽減できる。") {
			public void apply(Player p) { p.weapon.addEffect(new EffectImpactShot()); }});

		all.add(new PowerUp("Danmaku", "弾幕", "5発ばら撒き", "リロード増",
				"【解説】扇状に5発の弾を発射する。\n【運用】「面」での制圧が可能になる。\n相手の移動先を予測してばら撒き、回避ルートを限定させろ。") {
			public void apply(Player p) { p.weapon.addEffect(new EffectDanmaku()); }});

		all.add(new PowerUp("Reel Gun", "3点バースト", "3連射(乗算)", "リロード増",
				"【解説】1回の射撃で3発連続発射する。他の拡散効果と重複する。\n【運用】瞬間火力が高い。中距離で全弾命中させれば致命傷になる。\nリロードが重くなるため、残弾管理に注意。") {
			public void apply(Player p) { p.weapon.addEffect(new EffectReelGun()); }});

		all.add(new PowerUp("Shower", "シャワー", "15発拡散", "威力↓ リロード↑",
				"【解説】広範囲にランダム速度で多数の弾をばら撒く。\n【運用】狙いが甘くても当たるため、近〜中距離の押し付けが強力。\n「Hill（吸血）」と組み合わせると回復効率が良い。") {
			public void apply(Player p) { p.weapon.addEffect(new EffectShower()); }});

		all.add(new PowerUp("Reflect", "反射", "反射数+2 威力↑", "リロード増",
				"【解説】弾が壁で跳ね返る回数が増え、威力も微増する。\n【運用】障害物の多いマップで真価を発揮する。\n壁裏に隠れた敵を、反射を利用して一方的に攻撃せよ。") {
			public void apply(Player p) { p.weapon.addEffect(new EffectReflection()); }});

		all.add(new PowerUp("No Control", "制御不能", "反射+5 速度↑", "リロード増",
				"【解説】反射回数が大幅に増え弾速も上がるが、制御が難しい。\n【運用】狭い通路や部屋に撃ち込むことで弾幕地獄を作り出せる。\n自爆のリスクはないため、閉所での乱戦に強い。") {
			public void apply(Player p) { p.weapon.addEffect(new EffectOutOfControl()); }});

		all.add(new PowerUp("Idaten", "韋駄天", "移動・弾速2倍", "HP・威力低下",
				"【解説】移動速度と弾速が跳ね上がるが、非常に撃たれ弱くなる。\n【運用】被弾＝死と心得よ。常に動き回り、相手の死角を取り続けろ。\nヒット＆アウェイ戦法の極致。") {
			public void apply(Player p) { p.weapon.addEffect(new EffectIdaten()); p.applyPowerUpStats(); }});

		all.add(new PowerUp("Cold", "冷却弾", "相手速度-50%", "リロード増",
				"【解説】命中した相手の移動速度を一時的に半減させる。\n【運用】初弾を当てれば相手は回避困難になる。\nそこへ高威力の次弾を叩き込むコンボの始動として優秀。") {
			public void apply(Player p) { p.weapon.addEffect(new EffectColdShot()); }});

		all.add(new PowerUp("3 in 1", "三位一体", "威力・速度+100%", "弾数-2 リロード増",
				"【解説】基本性能が大幅に強化されるが、装弾数が激減する。\n【運用】「Big Capacity」等で弾数を補わないとすぐに弾切れになる。\nリロードの隙をカバーできるスキルとの併用が推奨される。") {
			public void apply(Player p) { p.weapon.addEffect(new Effect3in1()); }});

		all.add(new PowerUp("Poison", "毒", "DOTダメージ", "リロード増",
				"【解説】命中後、一定時間継続ダメージを与える。\n【運用】逃げ回る相手に対して有効。一度当てれば、\n障害物に隠れられてもHP差を広げることができる。") {
			public void apply(Player p) { p.weapon.addEffect(new EffectPoisonNew()); }});

		all.add(new PowerUp("Ghost", "ゴースト", "短時間壁貫通", "リロード増",
				"【解説】発射直後の短時間、壁をすり抜ける弾になる。\n【運用】壁を挟んで睨み合っている状況を打破できる。\n相手の安全地帯をなくし、一方的に攻撃を通せる。") {
			public void apply(Player p) { p.weapon.addEffect(new EffectGhostShot()); }});

		all.add(new PowerUp("Quick", "クイック", "リロード時間半減", "なし",
				"【解説】リロード時間が半分になる。\n【運用】地味だが強力。絶え間なく弾幕を張り続けられる。\n装弾数の少ない高火力武器の弱点を打ち消すのに最適。") {
			public void apply(Player p) { p.weapon.addEffect(new EffectQuickReload()); }});

		// --- ステータス変更系 ---

		all.add(new PowerUp("Big Boy", "巨大化", "HP増・巨大化", "被弾判定増",
				"【解説】HPが1.5倍になるが、当たり判定も2倍になる。\n【運用】回避を捨てて体力で受けるタンク戦術向け。\n「Build Up」と組み合わせれば、要塞のような耐久力を得る。") {
			public void apply(Player p) { p.weapon.addEffect(new EffectBigBoy()); p.applyPowerUpStats(); }});

		all.add(new PowerUp("Small Boy", "小型化", "回避・速度UP", "HP低下",
				"【解説】HPが半減するが、体が小さくなり回避しやすくなる。\n【運用】通常の弾幕を隙間ですり抜けられるようになる。\n操作精度に自信がある上級者向け。") {
		...
		all.add(new PowerUp("Tac. Reload", "戦術リロード", "防御時弾回復", "CD+2秒",
				"【解説】ガード成功時に弾薬が全回復する。\n【運用】撃ち切った後のリロード時間を踏み倒せる。\n「撃つ→ガード→即撃つ」というループが可能になる。") {
			public void apply(Player p) { p.hasSkillTacticalReload = true; }});

		all.add(new PowerUp("Exc. Defense", "専守防衛", "2秒後再発動 HP+", "CD+2秒",
				"【解説】HPが増え、ガード終了の2秒後にもう一度ガードが発動する。\n【運用】守りは固くなるが、自分の攻撃ターンが遅れる点に注意。\n「Poison」などでリードを奪ってから守る時に強い。") {
		...
		all.add(new PowerUp("The World", "\"世界\"", "防御時周囲弾消去", "CD+5秒",
				"【解説】ガード発動の瞬間、周囲の敵弾を消滅させる。\n【運用】「Shower」や「Danmaku」などの弾幕への完全なカウンター。\n囲まれた際の緊急回避手段としても極めて優秀。") {
